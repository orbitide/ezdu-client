...
export function AppSidebar() {
    ...
    return (
        <aside className="flex w-16 shrink-0 flex-col bg-card border-r border-border lg:w-64">
            {/* Logo */}
            <div className="flex h-16 items-center justify-center border-b border-border shrink-0 lg:justify-start lg:px-5">
                <Link href="/dashboard" className="flex items-center gap-2">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-primary text-black font-bold text-sm">
                        E
                    </div>
                    <span className="hidden font-semibold text-foreground text-lg lg:block">Ezdu</span>
                </Link>
            </div>
            ...
            <nav className="flex-1 overflow-y-auto py-3 space-y-4 px-2 lg:px-3">
                {NAV_SECTIONS.map((section) => (
                    <div key={section.label}>
                        <p className="hidden pb-1 px-3 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground lg:block">
                            {section.label}
                        </p>
            ...
            {/* User strip */}
            <div className="flex justify-center border-t border-border py-3 shrink-0 lg:block lg:px-4">
                <Link
                    href="/profile"
                    className="flex items-center rounded-lg hover:bg-muted p-1.5 -m-1.5 transition-colors lg:gap-3"
                >
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-primary to-blue-600 text-white text-xs font-bold">
                        {initials}
                    </div>
                    <div className="hidden flex-1 min-w-0 lg:block">
                        <p className="text-sm font-medium text-foreground truncate">{user?.name || 'লোড হচ্ছে...'}</p>
