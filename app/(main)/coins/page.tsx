            ...
            <span className="flex items-center gap-1 text-xs font-bold text-amber-400">
                {isLoading ? (
                    ...
                    <>
                        ...
                        {hasEnoughCoin ? `${toBangla(option.cost)} EC` : 'EC কম'}
                    </>
                )}
            </span>
        </button>
    ...
    return (
        ...
        <div className="space-y-5">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="flex-1 text-lg font-bold text-foreground">কয়েন</h1>
                    {/* Coin balance */}
                    <div className="flex items-center gap-1.5 rounded-xl border border-amber-500/20 bg-amber-500/10 px-3 py-1.5">
                        ...
                        <span className="text-sm font-bold text-amber-400">{toBangla(coin)} EC</span>
                    </div>
                </div>
                ...
                >
                    ...
                    <div className="text-center">
                        <p className="text-xl font-bold text-foreground">Pro ট্রাই করো</p>
                        <p className="mt-1 text-sm text-muted-foreground">EC দিয়ে সব প্রিমিয়াম ফিচার আনলক করো</p>
                    </div>
                ...
                <div className="space-y-2">
                    <p className="px-1 text-sm font-bold text-foreground">ইসি (EC) অর্জন করো</p>
                    <a
                        href={SITE.googlePlayUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-3.5 rounded-xl border border-border bg-card px-4 py-3.5 transition-colors hover:border-border hover:bg-muted/60"
                    >
                        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-muted text-primary">
                            <PlayCircle size={24} />
                        </div>
                        <div className="flex-1">
                            <p className="text-sm font-bold text-foreground">অ্যাড দেখে EC নাও</p>
                            <p className="mt-0.5 text-xs text-muted-foreground">মোবাইল অ্যাপে অ্যাড দেখে সর্বোচ্চ ৫ EC পর্যন্ত পাও</p>
                        </div>
                        <div className="flex shrink-0 items-center gap-1 rounded-full border border-primary/30 bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary">
